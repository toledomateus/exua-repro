...
def compute_exua_dirichlet(dirichlet_model: DirichletSASRec,
    ...
    with torch.enable_grad():
        ...
        U_E = dirichlet_uncertainty(alpha)
        loss = U_E.sum()
        loss.backward()
...
if __name__ == '__main__':
    ...
    if args.save_path is None:
        args.save_path = f'saved/sasrec_dirichlet_ue_{args.method}_{timestamp}.pth'
    ...
    try:
        ...
        with torch.no_grad():
            alpha_out = dirichlet_model.get_alpha(item_seq, item_seq_len)
            U_E = dirichlet_uncertainty(alpha_out)
