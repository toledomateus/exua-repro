...
if __name__ == '__main__':
    ...
    if args.checkpoint is None:
        ...
        def _cpu_load(*a, **kw):
            result = _orig_load(*a, **{**kw, 'map_location': 'cpu'})
            # If it's a RecBole checkpoint dict, force CPU in the config too
            if isinstance(result, dict) and 'config' in result:
                result['config']['device'] = torch.device('cpu')
            return result
        ...
        if args.mode == 'posthoc':
            ...
            print("\n── Evaluating baseline (Table 1 comparison) ──")
            test_result = evaluate(model, test_data, config, dataset)
            print_scores("SASRec + MCDropout", test_result)

            # DEFINE CALLBACKS FOR MCD
            def exua_wrapper(inter):
                return compute_exua(model, inter, tau=0.001)
                
            def unc_wrapper(inter):
                _, u_t, _, _ = mcd_uncertainty(model, inter)
                return u_t

            # CALL UNIVERSAL MURR
            murr_score = compute_universal_mURR(
                test_data, exua_wrapper, unc_wrapper, device, K=10
            )
