...
# ─────────────────────────────────────────────
# 5.  JUST ALETORIC ExUA — DIRICHLET VERSION  (S = 1)
# ─────────────────────────────────────────────
...
def compute_exua_dirichlet(dirichlet_model: DirichletSASRec,
    ...
    with torch.enable_grad():
        ...
        U_A = dirichlet_uncertainty(alpha)
        loss = U_A.sum()
        loss.backward()
...
def build_attribution(M_max_A, M_min_A):
    """
    A_dis = 0.5 * (1 -  M_max^A  +  M_min^A)
    """
    return 0.5 * (1.0 - M_max_A + M_min_A)
...
def uncertainty_aware_training_dirichlet(
    ...
    for epoch in range(max_epochs):
        ...
        for batch_idx, interaction in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                M_max_A, M_min_A = compute_exua_dirichlet(
                    dirichlet_model, interaction, tau=tau
                )
                A = build_attribution(M_max_A, M_min_A)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prep',          type=str,   default='default',
                        help='Data preparation method (for save path naming).')
    ...
    if args.save_path is None:
        args.save_path = f'saved/sasrec_dirichlet_ua_{args.method}_{timestamp}.pth'
    ...
    try:
        ...
        with torch.no_grad():
            alpha_out = dirichlet_model.get_alpha(item_seq, item_seq_len)
            U_A = dirichlet_uncertainty(alpha_out)
