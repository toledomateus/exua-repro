...
def compute_exua_ensemble(models: list, interaction, tau: float = TEMPERATURE):
    """
    Computes the ExUA attribution map for a Deep Ensemble based on Aleatoric Uncertainty (U_A).
    ...
    with torch.enable_grad():
        ...
        # Aleatoric Uncertainty (U_A): average entropy of each member's predictive distribution
        U_A = -(probs_stack * (probs_stack + eps).log()).sum(dim=-1).mean(dim=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help='"uncertainty" for analysis only, "posthoc" for posthoc training.'
    )
