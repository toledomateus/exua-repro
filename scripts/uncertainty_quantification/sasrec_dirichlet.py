...
import numpy as np
import os
from murr import compute_universal_mURR
...
def edl_loss(alpha: torch.Tensor, targets: torch.Tensor,
    ...
    # Bayes risk under cross-entropy (Sensoy et al. Eq. 4)
    ...
    # Final Bayes risk under cross-entropy (Sensoy et al. Eq. 4)
...
def compute_exua_dirichlet(dirichlet_model: DirichletSASRec,
    ...
    padding_mask = (item_seq_d == 0)
    # Mask m_plus before softmax (set pad tokens to -infinity)
    m_plus_masked = m_plus.masked_fill(padding_mask, float('-inf'))
    m_minus_masked = m_minus.masked_fill(padding_mask, float('-inf'))

    M_max = F.softmax(m_plus_masked  / tau, dim=-1)
    M_min = F.softmax(m_minus_masked / tau, dim=-1)
...
def generate_raw_signal_histogram(dirichlet_model, test_data, device, num_batches=10):
    """
    Visualization 2: Proves the 'Vanishing Saliency' flaw.
    Extracts the raw dot-product signal BEFORE the ReLU and Softmax 
    are applied to show the microscopic scale and the 50% wipeout.
    """
    print("\nGenerating Raw Signal Histogram (Visualization 2)...")
    dirichlet_model.eval()
    
    all_raw_signals =[]
    
    # We only need a few batches to get a massive statistical distribution 
    # (e.g., 10 batches * 2048 seqs * ~10 items = ~200,000 data points)
    for i, batch in enumerate(test_data):
        ...
        # --- Replicate ExUA forward/backward pass ---
        with torch.enable_grad():
            item_emb = dirichlet_model.sasrec.item_embedding(item_seq).detach()
            item_emb.requires_grad = True
            
            alpha = dirichlet_model.get_alpha_from_emb(item_emb, item_seq, item_seq_len)
            _, U_T, _, _ = dirichlet_uncertainty(alpha)
            loss = U_T.sum()
        ...
        # THIS is the raw signal the paper relies on
        grad_x_emb = (grad * item_emb).sum(dim=-1) # [B, T]
        
        # --- Remove padding tokens so they don't skew the histogram ---
        padding_mask = (item_seq == 0)
        valid_signals = grad_x_emb[~padding_mask].cpu().detach().numpy()
        
        all_raw_signals.extend(valid_signals)
        dirichlet_model.zero_grad()
    ...
    # Plot histogram
    sns.histplot(all_raw_signals, bins=100, color='lightgray', edgecolor='black', linewidth=0.5)
    
    # Draw a line at exactly 0.0 (The ReLU cutoff)
    plt.axvline(x=0, color='black', linestyle='--', linewidth=2)
    
    # Shade the regions to visually explain the math failure
    plt.axvspan(all_raw_signals.min(), 0, color='gray', alpha=0.3, 
                label='Destroyed by ReLU ($\sim$50% mapped to exactly 0)')
    plt.axvspan(0, all_raw_signals.max(), color='crimson', alpha=0.1, 
                label='Surviving Signal (Microscopic scale $\sim10^{-6}$)')
    
    plt.title("The Vanishing Saliency Trap: Raw ExUA Signals Before ReLU", fontsize=14, fontweight='bold')
    plt.xlabel("Gradient $\\times$ Embedding Dot Product Value", fontsize=12)
    plt.ylabel("Frequency (Number of Items)", fontsize=12)
    
    # Force scientific notation on X-axis to expose the tiny magnitude
    plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
    plt.legend(loc='upper right')
    plt.tight_layout()
    
    plt.savefig("exua_raw_signals_hist.pdf", dpi=300)
    print("Saved -> exua_raw_signals_hist.pdf")
...
def uncertainty_aware_training_dirichlet(
    ...
    for epoch in range(max_epochs):
        ...
        for batch_idx, interaction in enumerate(train_dataloader):
            ...
            item_emb_hat = (1.0 + alpha_scale * A.unsqueeze(-1)) * item_emb
...
if __name__ == '__main__':
    ...
    if args.mode == 'posthoc':
        ...
        # Uncertainty-only modeunc_wrapper
        ...
        # DEFINE CALLBACKS FOR DIRICHLET
        def exua_wrapper(inter):
            return compute_exua_dirichlet(dirichlet_model, inter, tau=0.001)
            
        def unc_wrapper(inter):
            seq = inter['item_id_list']
            seq_len = inter['item_length']
            target = inter['item_id']  # shape (B,) — already available!
            
            alpha = dirichlet_model.get_alpha(seq, seq_len)  # (B, M)
            
            # Gather only the target item's alpha
            target_alpha = alpha.gather(1, target.unsqueeze(1))  # (B, 1)
            
            # For a single item, uncertainty reduces to:
            # recompute over just [alpha_target, S0 - alpha_target] (binary)
            # OR simply use the target item's concentration directly
            S0 = alpha.sum(dim=-1, keepdim=True)          # (B, 1)
            alpha_candidates = torch.cat([target_alpha, S0 - target_alpha], dim=1)  # (B, 2)
            
            _, u_t, _, _ = dirichlet_uncertainty(alpha_candidates)
            return u_t  # (B,)
        # CALL UNIVERSAL MURR
        murr_score = compute_universal_mURR(
            test_data, exua_wrapper, unc_wrapper, device, K=5
        )
        # generate_raw_signal_histogram(dirichlet_model, test_data, device)
