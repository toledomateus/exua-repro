...
def load_ensemble(checkpoint_paths: list, device: str = 'cpu'):
    """
...
def compute_exua_ensemble(models: list, interaction, tau: float = TEMPERATURE):
    """
    Computes the ExUA attribution map for a Deep Ensemble based on Total Uncertainty.
    Gradients are enabled only for the forward+backward pass to compute saliency.
    Post-backward aggregation runs under no_grad to prevent building a useless graph
    from the still-live requires_grad=True leaf tensors (item_embs).
    """
    ref_device   = next(models[0].parameters()).device
    ...
    with torch.enable_grad():
        ...
        eps = 1e-10
        U_T = -(probs_mean * (probs_mean + eps).log()).sum(dim=-1)
...
def uncertainty_aware_training_ensemble(
    ...
    for epoch in range(max_epochs):
        ...
        for batch_idx, interaction in enumerate(train_dataloader):
            ...
            # Compute shared ExUA attention vector 
    ...
    if any(s is not None for s in best_states):
        ...
        for i, (model, state) in enumerate(zip(models, best_states)):
            ...
            path = os.path.join(save_dir, f'SASRec_DE_posthoc_member{i+1}.pth')
            torch.save({
                'config':          config,
...
if __name__ == '__main__':
    ...
    models, config, dataset, train_data, valid_data, test_data = load_ensemble(
        checkpoint_paths, device='cuda' if torch.cuda.is_available() else 'cpu'
    )
    ...
    if args.mode == 'posthoc':
        ...
        print_scores("ExUA-DE (after)", test_after)
        ...
        # DEFINE CALLBACKS FOR ENSEMBLES
        def exua_wrapper(inter):
            return compute_exua_ensemble(models, inter, tau=0.001)
            
        def unc_wrapper(inter):
            _, u_t, _, _ = de_uncertainty(models, inter)
            return u_t
