...
#     Now driven by Aleatoric Uncertainty (U_A)
# ─────────────────────────────────────────────
...
def compute_exua(model, interaction, n_samples: int = MCD_SAMPLES,
    ...
    Attribution signal: Aleatoric Uncertainty (U_A).
    U_A = E_theta[ H(p(y|x, theta)) ]  — the expected per-sample entropy,
    which captures the irreducible data-level uncertainty rather than the
    model-parameter uncertainty captured by U_T.
    """
    model.eval()
    enable_dropout(model)
    ...
    for _ in range(n_samples):
        ...
        with torch.enable_grad():
            ...
            eps = 1e-10
            # ── KEY CHANGE ────────────────────────────────────────────────
            # Use per-sample entropy (U_A component) instead of the entropy
            # of the mean distribution (U_T).  We backprop the per-sample
            # Shannon entropy H(p | theta_t) and average across MCD samples,
            # approximating E_theta[H(p(y|x,theta))].
            # ─────────────────────────────────────────────────────────────
            per_sample_entropy = -(probs * (probs + eps).log()).sum(dim=-1)

            # Backprop sum of per-sample entropies (aleatoric signal)
            loss = per_sample_entropy.sum()
...
if __name__ == '__main__':
    ...
    if args.save_path is None:
        args.save_path = f'saved/sasrec_mcd_ua_{timestamp}.pth'
    ...
    if args.checkpoint is None:
        ...
        # Ensure the config includes the new metrics
        config['metrics'] = parameter_dict['metrics']

        # Override config device with the auto-detected one
        ...
        if args.mode == 'posthoc':
            ...
            # ── Post-hoc training ─────────────────────────────────────────
            print("\n── Starting uncertainty-aware post-hoc training (U_A signal) ──")
            ...
            print_scores("ExUA-MCD U_A (after)", test_after)
